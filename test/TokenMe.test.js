...
const TokenMe = artifacts.require('TokenMe.sol');

// Based on OpenZeppelin tests - MIT licence
// https://github.com/OpenZeppelin/openzeppelin-solidity
contract('TokenMe', function ([
  creator,
  ...accounts
]) {
  const name = 'TokenMe';
  ...
  beforeEach(async function () {
    this.token = await TokenMe.new({ from: creator });
  });
  ...
  describe('like an ERC721', function () {
    ...
    describe('metadata', function () {
      const baseUri = 'https://abcoathup.github.io/tokenme/apisampledata/tokenme/';
      ...
      it('reverts when querying metadata for non existent token id', async function () {
        await expectRevert(
          this.token.tokenURI(nonExistentTokenId), 'TokenMe: URI query for nonexistent token'
        );
      });
    });
  });
